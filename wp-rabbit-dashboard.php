...
function custom_dashboard_widget_display() {
    echo '<h3 class="sub-head">WP Rabbit Dashboard!</h3>';
        ...
        fetch(`https://rabbitwebdesign.co.uk/wp-json/custom-api/v1/dashboard-data/?client_id=${clientID}`)
            ...
            .then(data => {
                ...
                const isValidUrl = (string) => {
                  ...
                  // Add image if valid
                if (data.btnprimary && isValidUrl(data.btnprimary)) {
                    content += `<p><button href="${data.btnprimary}">click</button></p>`;
                }
